...
def process_dropapk_url(data,apksavename,proxy):
    ...
    if proxy:
        text = s.post(url,data = datas,proxies=proxy).text
    else:
...
def process_apptoko_url(the_job,save_path,proxy):
    ...
    if os.path.exists(file_name):
        ...
        return False
def process_android1_url(the_job,save_path,proxy):
    s = requests.session()
    if proxy:
        text = s.get(the_job[2], proxies=proxy).text
    else:
        text = s.get(the_job[2]).text
    re_com = re.compile(r'id=download></div><a href=(.*?)>')
    url_down = re.search(re_com, text).group(1).replace('amp;', '')
    print url_down
    if ".google.com" in url_down:
        newtext = s.get(url_down, proxies=proxy).text
        re_com = re.compile(r'class="goog-inline-block jfk-button jfk-button-action" href="(.*?)">')
        url_down = re.search(re_com, newtext).group(1).replace('amp;', '')
        if "drive.google.com" in url_down:
            domians = "https://drive.google.com"
        else:
            domians = "https://docs.google.com"
        urls = domians + url_down
        if ".rar" in newtext:
            ext = ".rar"
        elif ".zip" in newtext:
            ext = ".zip"
        else:
            print "Unknow Download Type"
            return False
        downloadtext = s.get(urls, proxies=proxy).text
        re_com = re.compile(r'class="goog-inline-block jfk-button jfk-button-action" href="(.*?)">')
        url_down = re.search(re_com, newtext).group(1).replace('amp;', '')
        urls = domians + url_down
        print urls
        file_name = save_path + the_job[1] + ext
        print file_name
        with closing(s.get(urls, stream=True,proxies=proxy)) as r:
            with open(file_name, "wb") as code:
                for chunk in r.iter_content(chunk_size=512000):
...
elif "android-1.com" in the_job[2]:
    savepath = process_android1_url(the_job,save_path,proxy)
