...
def get_proxy():
    ...
    cursor = db.cursor()
    sql = "select id,proxy,counts,faild from proxy_google_tag_test order by status asc limit 1 "
    try:
        cursor.execute(sql)
        proxies = cursor.fetchone()
        counts = proxies[2] + 1
        # 更新代理时间戳
        sql = "update proxy_google_tag_test set status=%d,counts=%d where id=%d" % (int(time.time()),counts, proxies[0])
        cursor.execute(sql)
        db.commit()
    except Exception, e:
        db.rollback()
...
def get_path():
    ...
    try:
        ...
        if data:
            ...
            print id
            # 数据取出后改一下状态，表示开始处理
            sql = "update png_center_pdts set tag_status=3 where id=%d" % id
            try:
                cursor.execute(sql)
                db.commit()
...
def goole_vision():
    ...
    if path_id:
        ...
        try:
            ...
            proxy_sql = "update proxy_google_tag_test set faild=%d，counts=%d where id=%d " % (proxy_faild,proxy[2],proxy[0])
            cursor.execute(proxy_sql)
            db.commit()
            sql = "update png_center_pdts set tag_status = -13 where id=%d " % id
            cursor.execute(sql)
            db.commit()
            db.close()
            print "Proxy Error"
        ...
        db = MySQLdb.connect('localhost', 'aio_music', 'aio_music_pass', 'kiss_png')
        cursor = db.cursor()
        labels_new = labels.replace('"',' ')
        if labels:
            print "Get Tag Success"
            sql  = 'update png_center_pdts set tag_status=15,lab_tag="%s",web_tag="%s" where id=%d' % (labels_new,webEntities,id)
        else:
            #更改代理表
            proxy_faild = proxy[3] + 1
            proxy_sql = "update proxy_google_tag_test set faild=%d，counts=%d where id=%d " % (proxy_faild,proxy[2],proxy[0])
